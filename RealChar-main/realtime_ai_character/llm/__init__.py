...
def get_llm(model="localhost") -> LLM:
    ...
    elif "LDN_MODEL" in model:
        from realtime_ai_character.llm .ldn_llm import ldnllm
        return ldnllm()
...
def get_chat_model(model="localhost") -> BaseChatModel:
    model = os.getenv("LLM_MODEL_USE", model)
